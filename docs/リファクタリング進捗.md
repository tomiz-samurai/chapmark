...
3. **コンポーネント型更新**
   ...
   - ✅ `Button`/`BookCard`などの共通コンポーネントから`as any`/`as 型名`の撲滅・型安全化
   - ✅ Featherアイコン名の型安全化（`icon?: keyof typeof Feather.glyphMap`）
   - ✅ BookCardのImageSourcePropType型アサーション排除
...
1. **TimerScreenの改善**
   ...
   - ✅ useBookSelectionで本選択時の元ステータス（originalStatus）をuseRefで保持
   - ✅ useReadingSession/TimerScreenでoriginalStatusを受け渡し、タイマー完了時に元のstatusを考慮
...
3. **BookServiceとTimerServiceの連携・責務明確化**
   ...
   - ✅ completeReadingSessionAsyncの引数にoriginalStatusを追加し、元がcompletedなら必ずcompletedに戻すロジックに修正（タイマーと本棚のステータス同期バグ修正）
...
## TODO（残タスク）
- [x] サービス層のテスト用モック・スタブ設計と実装
  - BookService, TimerService等の主要メソッドを型安全なJest用モックとしてlib/services/__mocks__に実装済み。テストで外部依存を切り離して検証可能な状態を確認。
- [x] 依存関係のさらなる明確化（DIパターン等の導入・循環参照の根本解消）
  - サービス層の依存注入（DI）設計を導入、もしくは依存関係を明文化
  - テストや将来の差し替えが容易な構造になっていることを確認
- [x] CIでの型安全チェック・テスト体制の強化
  - CI（GitHub Actions等）で型チェック（tsc --noEmit）・lint・ユニットテストが自動実行される
  - main/masterマージ時に型エラーやテスト失敗がブロックされる
- [x] 今後API拡張時のDTO型・API設計ルールの運用徹底
  - types/dto配下でDTO型（例: BookCreateRequest, BookResponse）を管理し、API追加時は必ず型定義・ドキュメントを更新する運用を開始
  - API設計ルール（docs/リファクタリング方針.md）を遵守し、型定義・API仕様変更時は必ずドキュメントも更新
- [ ] ドキュメントのさらなる充実・運用ルールの定期見直し
  - 設計方針・運用ルール・API仕様・テスト方針などがdocs/配下に整理されている
  - ルールや設計変更時は必ずドキュメントを更新し、定期的に見直しを実施
...
### 📝 今後必須のリファクタリングTODO（優先順位順）

1. 旧型定義・any型・Omit型の完全撲滅（全体の型・import揺れ撲滅）
2. import経路・型exportの一元化（types/models, types/dto経由に統一）
3. 共通コンポーネントの拡充・責務分割の徹底
4. スタイル・テーマの一元管理徹底
5. パフォーマンス最適化（再レンダリング抑制・セレクタ見直し）
6. テスト体制の強化（ユニット・E2E・CI自動検証）
7. ドキュメント・運用ルールの継続的整備
8. 将来拡張に備えたサービス層・設計の再点検

### 2024-06-13 進捗追記

- HomeScreen（app/(tabs)/index.tsx）でBookServiceの直接呼び出しを廃止し、ReduxのThunk/Selector経由で書籍データ取得に統一
- linterエラーも解消済み
- 他画面・フックでもBookService/TimerServiceの直接呼び出しがないか再確認し、同様の統一を進行中

### 2024-06-13 進捗追記③

- BookService/TimerServiceのAPI引数・戻り値の型注釈がすべてtypes/models配下の型で統一されていることを最終確認
- サービス層の型安全・型定義一元管理が完了
- 型定義・API設計ルールもdocs/リファクタリング方針.mdに明文化済み

### 2024-06-13 進捗追記④

- フェーズ3「サービスアーキテクチャの改善」：
  - TimerService/BookServiceのRedux連携・責務分離・副作用一元化 完了
  - サービスメソッドの型定義・API一元管理 完了
  - 型定義・API設計ルールのドキュメント化 完了
- ChapMarkのサービスアーキテクチャは現時点でモダンかつ拡張性・保守性の高い状態に到達

### 2024-06-XX 進捗追記⑤

- サービス層（TimerService/NotificationService）の依存注入（DI）設計を徹底
  - TimerServiceClassはstoreをconstructorで受け取る形にリファクタ
  - ServiceContainerでstore生成後にTimerServiceへDI
  - store→ServiceContainer→TimerServiceの依存の一方向化を実現
  - require cycle（循環参照）・初期化タイミングの問題を完全解消
- Redux ThunkのextraArgumentもServiceContainer経由で統一
- 全画面・全サービスでシングルトンimportを廃止し、DI経由での利用に統一
- これにより「default export missing」や「prototype of undefined」等の致命的エラーも解消
...
- フェーズ2: コンポーネント構造のリファクタリング（完了）
- フェーズ3: サービスアーキテクチャの改善（完了）
- 依存関係の明確化・DI設計の徹底（循環参照の根本解消・全サービスのDI化） 